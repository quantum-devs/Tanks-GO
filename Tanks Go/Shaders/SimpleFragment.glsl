...
void main(void) {
    ...
    highp float DiffuseFactor = max(-dot(Normal, u_Light.Direction), 0.0);
    highp vec3 DiffuseColour = u_Light.Colour * u_Light.DiffuseIntensity * DiffuseFactor;
    
    //Specular
    highp vec3 Eye = normalize(frag_Position);
    highp vec3 Reflection = reflect(u_Light.Direction, Normal);
    highp float SpecularFactor = pow(max(0.0, -dot(Reflection, Eye)), u_Shininess);
    highp vec3 SpecularColour = u_Light.Colour * u_MatSpecularIntensity * SpecularFactor;
